...
public extension SyncCache {
    ...
    func mapID<OtherID: Hashable>(_ transform: @escaping (OtherID) -> ID) -> some SyncCache<OtherID, Value> {
        AnySyncCache { id in
            ...
            store(value: value, for: transform(id))
        }
    }
}
...
public extension SendableSyncCache {
    /**
     ...
     - Returns: A cache that takes `OtherID` as its `ID` type.
     */
    func mapID<OtherID: Hashable>(_ transform: @escaping @Sendable (OtherID) -> ID) -> some SyncCache<OtherID, Value> {
        AnySendableSyncCache { id in
            value(for: transform(id))
        } storeValueForID: { value, id in
            store(value: value, for: transform(id))
        }
...
public extension AsyncCache {
    ...
    func mapID<OtherID: Hashable & Sendable>(
        ...
        AnyAsyncCache { id in
            ...
            await store(value: value, for: transform(id))
        }
    }
}
